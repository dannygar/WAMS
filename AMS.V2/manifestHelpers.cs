...
namespace AMS.V2
{
    public class ManifestHelpers
    {
        ...
        public static ManifestTimingData GetManifestTimingData(CloudMediaContext context, IAsset asset, ILogger log)
        // Parse the manifest and get data from it
        {
            ...
            try
            {
                ...
                if (myuri != null)
                {
                    log.LogInformation($"Asset URI {myuri.ToString()}");

                    XDocument manifest = XDocument.Load(myuri.ToString());

                    //log.LogInformation($"manifest {manifest}");
                    ...
                    foreach (var chunk in videotrack.Elements("c"))
                    {
                        ...
                        log.LogInformation($"duration d {durationchunk}");

                        repeatchunk = chunk.Attribute("r") != null ? int.Parse(chunk.Attribute("r").Value) : 1;
                        log.LogInformation($"repeat r {repeatchunk}");
                        ...
                        if (chunk.Attribute("t") != null)
                        {
                            ...
                            log.LogInformation($"t value {ulong.Parse(chunk.Attribute("t").Value)}");
                        }
                        else
                        {
        ...
        public static async Task<EndTimeInTable> RetrieveLastEndTimeAsync(CloudTable table, string programID)
        {
        ...
        public static async Task UpdateLastEndTimeAsync(CloudTable table, TimeSpan endtime, string programId, int id, ProgramState state)
        {
